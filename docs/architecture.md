...
| OIDC / Keycloak integration | `apps/shell/src/auth/` | injected as props |
...
`authSlice` (non-API) stores the access token and is persisted to `localStorage`.
...
```
packages/app-core/src/constants/tools.ts   — tool definition (id, type, endpoint, …)
        ↓
useTransformText hook (editor-remote)      — wraps transformText mutation from textApi
        ↓
...
TextResponse { original, result, operation }
        ↓
useTransformText                           — returns result to TextForm in editor-remote
```
