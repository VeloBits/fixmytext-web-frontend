...
| content          | `@velobits/content-app`      | 3103                | 3103             |

> Note: `apps/content` uses 3103 everywhere (bare local dev, Docker dev, and the Cloudflare prod simulation). It is currently parked — the shell owns the origin root, so the router at 3100 does not proxy it; Traefik still sends the marketing paths straight to 3103.
